...
const contractObject = async () => {
    ...
    console.log("pool info address", object)
...
export async function contractData(address) {
    try {
        ...
        console.log("pool info address", contractObj)
...
export const addTokenToMetamask = async () => {
    if (window.ethereum) {
        ...
        const tokenDecimals = await tokenContractObj.decimals();
        const tokenAddress = await tokenContractObj.address;
        const tokenSymbol = await tokenContractObj.symbol();
...
export const buyToken = async (amount) => {
    try {
        ...
        const contractObj = await tokenIcoContract(amount);
        const tokenDetails = await contractObj.getTokenDetails();
...
export const tokenWithdraw = async () => {
    try {
        notifySuccess("Calling ico contract ");
        const contractObj = await tokenIcoContract();
        const tokenDetails = await contractObj.getTokenDetails();
        const availableToken = ethers.utils.formatEther(tokenDetails.balance.toString());

        if (availableToken > 1) {
            const transaction = await contractObj.withdrawAllTokens();
...
export const updateToken = async (_address) => {
    try {
        ...
        const transaction = await contractObj.updateToken(_address, {
            gasLimit: gasEstimation,
        });
